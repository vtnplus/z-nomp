...
module.exports = function(logger, poolConfig){
    ...
    var accountdb_in = (accountdb ? '`' + accountdb + '`.' : "");
    var workerdb_in = (workerdb ? '`' + workerdb + '`.' : "");
                        ...
                        connection.query(
                            ...
                            function(err, result){
                                if (err){
                                    ...
                                    connection.query(
                                        "INSERT INTO ' + workerdb_in + '`pool_worker` (`account_id`, `username`, `password`) VALUES (?, ?, ?);",
    ...
    this.handleDifficultyUpdate = function(workerName, diff){
        ...
        connection.query(
            'UPDATE ' + workerdb_in + '`pool_worker` SET `difficulty` = ' + diff + ' WHERE `username` = ' + connection.escape(workerName),
            function(err, result){
                if (err)
                    logger.error(logIdentify, logComponent, 'Error when updating worker diff: ' +
                        JSON.stringify(err));
                else if (result.affectedRows === 0){
                    connection.query('INSERT INTO ' + workerdb_in + '`pool_worker` SET ?', {username: workerName, difficulty: diff});
                }
                else
